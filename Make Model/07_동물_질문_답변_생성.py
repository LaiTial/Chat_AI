...
def read_data():
    route = 'Data/AnimalData.csv'
    csv = pd.read_csv(route, encoding = 'utf-8')
    csv.columns=['Intent', 'NER', 'Q', 'A']
...
def preprocess_qa(QALine):
  ...
  Intent, NER, Q, A = QALine

  return f"<intent>{Intent}<ner>{NER}<q>{Q}<a>{A}</s>"
...
training_args=TrainingArguments(
    ...
    metric_for_best_model="loss",  # perplexity를 평가 지표로 설정
    greater_is_better=False, # 평가지표가 낮게 모델을 조정
    lr_scheduler_type="constant_with_warmup",  # lr 스케줄러 지정
    warmup_ratio=0.4, # 전체 훈련 스텝에 대한 초기 학습률의 비율
    warmup_steps=0, # 웜업 스텝의 수
    dataloader_drop_last=False,
)

early_stopping_callback = EarlyStoppingCallback(early_stopping_patience=3, 
                                                early_stopping_threshold=.5)  # 평가지표가 20 이상 감소해야 조기 종료 조건을 충족
